...
В текущей версии production-контейнер перед API выполняет `alembic upgrade head`; `init_db()` вызывает `create_all()` только в development. Это подходит для новой базы, но не является автоматическим преобразованием уже существующей базы.

Для базы, которая была создана предыдущей версией через `create_all()`, сначала нужно сделать backup и вручную сравнить схему с `0001_initial_schema`. Если схема совпадает, оператор может один раз выполнить `alembic stamp 0001_initial_schema`, после чего следующие изменения будут идти обычными миграциями. Нельзя запускать `upgrade head` на существующей немигрированной базе без такой проверки: начальная миграция попытается создать уже существующие таблицы.
